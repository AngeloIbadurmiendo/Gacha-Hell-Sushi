...
app.get('/productos/categoria/:categoriaId', async (req, res) => {
    try {
        ...
        const productosCollection = client.db().collection('productos');
        const productos = await productosCollection.find({ categoria_id: new ObjectId(categoriaId) }).toArray();
        res.status(200).json(productos);
    } catch (error) {
        console.error('Error al obtener productos por categoría:', error);
        res.status(500).json({ message: 'Error interno del servidor al obtener productos por categoría' });
    }
});
...
app.post('/productos', async (req, res) => {
    try {
        const { nombre, imagenURL, precioBase, descuento, disponibilidad, descripcion, categoria_id } = req.body;
        if (!nombre || precioBase === undefined || !categoria_id) {
            return res.status(400).json({ message: 'Faltan campos obligatorios: nombre, precioBase, categoria_id' });
        }
...
app.put('/productos/:productoId', async (req, res) => {
    try {
        ...
        if (updates.precioBase !== undefined) updateDoc.precioBase = Number(updates.precioBase);
        if (updates.descuento !== undefined) updateDoc.descuento = Number(updates.descuento);
        if (updates.disponibilidad !== undefined) updateDoc.disponibilidad = Number(updates.disponibilidad);
        if (updates.descripcion !== undefined) updateDoc.descripcion = updates.descripcion;
        if (updates.categoria_id !== undefined) {
            if (!ObjectId.isValid(updates.categoria_id)) {
                return res.status(400).json({ message: 'ID de categoría inválido en la actualización' });
            }
...
app.delete('/productos/:productoId', async (req, res) => {
    try {
        ...
        const productosCollection = client.db().collection('productos');
        const result = await productosCollection.deleteOne({ _id: new ObjectId(productoId) });
...
app.get('/categorias/:categoriaId', async (req, res) => {
    try {
        ...
        const categoriasCollection = client.db().collection('categorias');
        const categoria = await categoriasCollection.findOne({ _id: new ObjectId(categoriaId) });
...
app.post('/categorias', async (req, res) => {
    try {
        ...
        if (!categoria) {
            return res.status(400).json({ message: 'El campo "categoria" es obligatorio' });
        }
        const categoriasCollection = client.db().collection('categorias');
        const newCategory = {
            categoria,
        ...
        res.status(201).json({ message: 'Categoría creada exitosamente', categoriaId: result.insertedId });
    } catch (error) {
...
app.delete('/categorias/:categoriaId', async (req, res) => {
    try {
        ...
        const categoriasCollection = client.db().collection('categorias');
        const result = await categoriasCollection.deleteOne({ _id: new ObjectId(categoriaId) });
        if (result.deletedCount === 0) {
...
app.post('/usuarios/registro', async (req, res) => {
    try {
        const { nombre, apellido, nombreUsuario, correo, contrasenia, numeroTelefonico } = req.body;
        if (!nombre || !correo || !contrasenia) {
            return res.status(400).json({ message: 'Faltan campos obligatorios: nombre, correo, contrasenia' });
        }
...
app.post('/usuarios/login', async (req, res) => {
    try {
        ...
        if (!correo || !contrasenia) {
            return res.status(400).json({ message: 'Faltan campos obligatorios: correo, contrasenia' });
        }
        ...
        if (contrasenia === user.contrasenia) { // Comparación directa (NO SEGURO PARA PRODUCCIÓN)
...
app.put('/usuarios/perfil', async (req, res) => {
    try {
        ...
        if (updates.nombreUsuario !== undefined) updateDoc.nombreUsuario = updates.nombreUsuario;
        ...
        if (updates.numeroTelefonico !== undefined) updateDoc.numeroTelefonico = updates.numeroTelefonico;
...
app.post('/usuarios/logout', async (req, res) => {
    res.status(200).json({ message: 'Sesión cerrada (operación simulada sin JWT)' });
});
...
app.get('/usuarios/direcciones', async (req, res) => {
    try {
        ...
        const direcciones = await direccionesCollection.find({ usuario_id: new ObjectId(userId) }).toArray();
...
app.get('/usuarios/direcciones/:direccionId', async (req, res) => {
    try {
        ...
        const userId = req.query.userId; // Espera userId como ?userId=...
        if (!ObjectId.isValid(direccionId) || !userId || !ObjectId.isValid(userId)) {
            return res.status(400).json({ message: 'ID de dirección o usuario inválido' });
        }
        const direccionesCollection = client.db().collection('direcciones');
        const direccion = await direccionesCollection.findOne({ _id: new ObjectId(direccionId), usuario_id: new ObjectId(userId) });

        if (direccion) {
            res.status(200).json(direccion);
        } else {
            res.status(404).json({ message: 'Dirección no encontrada o no pertenece al usuario' });
        }
    } catch (error) {
        console.error('Error al obtener la dirección:', error);
        res.status(500).json({ message: 'Error interno del servidor al obtener la dirección' });
    }
});
...
app.post('/usuarios/direcciones', async (req, res) => {
    try {
        const { usuario_id, calle, numero, departamento, comuna, predeterminado } = req.body;
        if (!usuario_id || !ObjectId.isValid(usuario_id) || !calle || !numero || !comuna) {
            return res.status(400).json({ message: 'Faltan campos obligatorios o ID de usuario inválido: usuario_id, calle, numero, comuna' });
        }
        const direccionesCollection = client.db().collection('direcciones');
        const newDireccion = {
        ...
        res.status(201).json({ message: 'Dirección agregada exitosamente', direccionId: result.insertedId });
    } catch (error) {
        console.error('Error al agregar dirección:', error);
        res.status(500).json({ message: 'Error interno del servidor al agregar la dirección' });
    }
});
...
app.put('/usuarios/direcciones/:direccionId', async (req, res) => {
    try {
        const direccionId = req.params.direccionId;
        const { usuario_id, calle, numero, departamento, comuna, predeterminado } = req.body;
        if (!ObjectId.isValid(direccionId) || !usuario_id || !ObjectId.isValid(usuario_id)) {
            return res.status(400).json({ message: 'ID de dirección o usuario inválido' });
        }
        ...
        if (updates.departamento !== undefined) updateDoc.departamento = updates.departamento;
        if (updates.comuna !== undefined) updateDoc.comuna = updates.comuna;
        if (updates.predeterminado !== undefined) updateDoc.predeterminado = updates.predeterminado;
        ...
        res.status(500).json({ message: 'Error interno del servidor al actualizar la dirección' });
    }
});
...
app.delete('/usuarios/direcciones/:direccionId', async (req, res) => {
    try {
        ...
        const direccionesCollection = client.db().collection('direcciones');
        const result = await direccionesCollection.deleteOne({ _id: new ObjectId(direccionId), usuario_id: new ObjectId(usuario_id) });

        if (result.deletedCount === 0) {
            return res.status(404).json({ message: 'Dirección no encontrada o no pertenece al usuario para eliminar' });
        }
        res.status(200).json({ message: 'Dirección eliminada exitosamente' });
    } catch (error) {
        console.error('Error al eliminar dirección:', error);
        res.status(500).json({ message: 'Error interno del servidor al eliminar la dirección' });
    }
});
...
app.post('/pedidos', async (req, res) => {
    try {
        const { usuario_id, direccionEnvio_id, metodoPago, costoDespacho, items } = req.body;
        if (!usuario_id || !ObjectId.isValid(usuario_id) || !direccionEnvio_id || !ObjectId.isValid(direccionEnvio_id) || !metodoPago || !items || !Array.isArray(items) || items.length === 0) {
            return res.status(400).json({ message: 'Faltan campos obligatorios o datos inválidos para el pedido' });
        }
        ...
        const productosEnPedido = await productosCollection.find({ _id: { $in: productoIds } }).toArray();
        const productosMap = new Map(productosEnPedido.map(p => [p._id.toString(), p]));

        for (const item of items) {
            if (!ObjectId.isValid(item.productoId) || !item.cantidad || item.cantidad <= 0) {
                return res.status(400).json({ message: 'Ítem de pedido inválido: productoId o cantidad' });
            }
            ...
            const precioUnitario = producto.precioBase - (producto.precioBase * (producto.descuento / 100 || 0));
        ...
        res.status(201).json({ message: 'Pedido creado exitosamente', pedidoId: resultPedido.insertedId });
...
app.get('/pedidos', async (req, res) => {
    try {
        ...
        const pedidosCollection = client.db().collection('pedidos');
        const pedidos = await pedidosCollection.find({ usuario_id: new ObjectId(userId) }).toArray();
...
app.get('/pedidos/:pedidoId', async (req, res) => {
    try {
        ...
        if (!ObjectId.isValid(pedidoId) || !userId || !ObjectId.isValid(userId)) {
            return res.status(400).json({ message: 'ID de pedido o usuario inválido' });
        }

        const pedidosCollection = client.db().collection('pedidos');
        const pedido = await pedidosCollection.findOne({ _id: new ObjectId(pedidoId), usuario_id: new ObjectId(userId) });

        if (pedido) {
            res.status(200).json(pedido);
        } else {
            res.status(404).json({ message: 'Pedido no encontrado o no pertenece al usuario' });
        }
    } catch (error) {
        console.error('Error al obtener el pedido:', error);
        res.status(500).json({ message: 'Error interno del servidor al obtener el pedido' });
    }
});
...
app.put('/pedidos/:pedidoId', async (req, res) => {
    try {
        ...
        if (updates.despacho_id !== undefined) {
            if (updates.despacho_id !== null && !ObjectId.isValid(updates.despacho_id)) {
                return res.status(400).json({ message: 'ID de despacho inválido' });
            }
            updateDoc.despacho_id = updates.despacho_id ? new ObjectId(updates.despacho_id) : null;
        }
...
app.post('/pedidos/:pedidoId/anular', async (req, res) => {
    try {
        ...
        if (!motivoAnulacion) {
            return res.status(400).json({ message: 'Se requiere un motivo de anulación' });
        }

        const pedidosCollection = client.db().collection('pedidos');
        const result = await pedidosCollection.updateOne(
            { _id: new ObjectId(pedidoId) },
            { $set: { estado: 'Anulado', motivoAnulacion: motivoAnulacion, fechaAnulacion: new Date() } }
        );

        if (result.matchedCount === 0) {
            return res.status(404).json({ message: 'Pedido no encontrado para anular' });
        }
...
app.get('/despachos/:despachoId', async (req, res) => {
    try {
        ...
        const despacho = await despachosCollection.findOne({ _id: new ObjectId(despachoId) });
        ...
        res.status(500).json({ message: 'Error interno del servidor al obtener el despacho' });
    }
});
...
app.put('/despachos/:despachoId', async (req, res) => {
    try {
        ...
        const updateDoc = {};
        if (updates.estado !== undefined) updateDoc.estado = updates.estado;
        if (updates.detallesSeguimiento !== undefined) updateDoc.detallesSeguimiento = updates.detallesSeguimiento;
        ...
        if (result.matchedCount === 0) {
            return res.status(404).json({ message: 'Despacho no encontrado para actualizar' });
        }
        res.status(200).json({ message: 'Despacho actualizado exitosamente' });
    } catch (error) {
        console.error('Error al actualizar el despacho:', error);
        res.status(500).json({ message: 'Error interno del servidor al actualizar el despacho' });
    }
});
...
app.get('/ingredientes', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error interno del servidor al obtener ingredientes' });
    }
});
...
app.get('/ingredientes/:ingredienteId', async (req, res) => {
    try {
        ...
        const ingredientesCollection = client.db().collection('ingredientes');
        const ingrediente = await ingredientesCollection.findOne({ _id: new ObjectId(ingredienteId) });
        ...
        res.status(500).json({ message: 'Error interno del servidor al obtener el ingrediente' });
    }
});
...
app.post('/ingredientes', async (req, res) => {
    try {
        ...
        if (!nombre || cantidad === undefined || !unidad) {
            return res.status(400).json({ message: 'Faltan campos obligatorios: nombre, cantidad, unidad' });
        }
        const ingredientesCollection = client.db().collection('ingredientes');
        ...
        res.status(201).json({ message: 'Ingrediente creado exitosamente', ingredienteId: result.insertedId });
    } catch (error) {
...
app.put('/ingredientes/:ingredienteId', async (req, res) => {
    try {
        ...
        const updateDoc = {};
        if (updates.nombre !== undefined) updateDoc.nombre = updates.nombre;
        if (updates.cantidad !== undefined) updateDoc.cantidad = Number(updates.cantidad);
...
app.delete('/ingredientes/:ingredienteId', async (req, res) => {
    try {
        ...
        const ingredientesCollection = client.db().collection('ingredientes');
        const result = await ingredientesCollection.deleteOne({ _id: new ObjectId(ingredienteId) });
